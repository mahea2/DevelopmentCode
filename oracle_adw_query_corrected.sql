-- Oracle ADW Query with corrected column names
-- Common alternatives for INVDATE: INV_DATE, INVOICE_DATE, DOC_DATE, DATE_CREATED
-- Common alternatives for EXPDATE: EXP_DATE, EXPIRY_DATE, DUE_DATE

SELECT 
    TO_CHAR(TO_DATE(IH.INV_DATE, 'YYYYMMDD'), 'DD/MM/YY') AS INVDATE,  -- Try INV_DATE instead of INVDATE
    'IN' AS TYPE,
    INVNUMBER,
    IH.SHINUMBER,
    IH.ORDNUMBER,
    TO_CHAR(TO_DATE(IH.EXP_DATE, 'YYYYMMDD'), 'DD/MM/YY') AS EXPDATE,  -- Try EXP_DATE instead of EXPDATE
    PONUMBER,
    REFERENCE,
    IH.DESCRIPTION,
    CUSTOMER,
...
FROM SFLDAT.OEINVH IH
    ...
    LEFT JOIN (
        ...
        WHERE CP.Item_No = ID.ITEM
            AND TO_DATE(IH.INV_DATE, 'YYYYMMDD') BETWEEN CP.Start_Date AND CP.End_Date  -- Updated column name
            AND (
    ...
    LEFT JOIN (
        ...
        WHERE CD.Item_No = ID.ITEM
            AND TO_DATE(IH.INV_DATE, 'YYYYMMDD') BETWEEN CD.Start_Date AND CD.End_Date  -- Updated column name
            AND (
    ...
    LEFT JOIN (
        ...
        WHERE I.ITEMNO = ID.ITEM
            ...
            AND I.DPRICETYPE = 2
            AND I.CURRENCY = 'NZD'
            AND TO_DATE(IH.INV_DATE, 'YYYYMMDD') BETWEEN I.SALESTART AND I.SALEEND  -- Updated column name
    ...
    LEFT JOIN (
        ...
        WHERE CON.CUSTNO = IH.CUSTOMER
            ...
            AND TO_DATE(IH.INV_DATE, 'YYYYMMDD') BETWEEN CON.STARTDATE AND CON.EXPIRE  -- Updated column name
            AND CON.CUSTTYPE = 1
    ) CONTRACT_PRICE ON CONTRACT_PRICE.CUSTNO = IH.CUSTOMER AND CONTRACT_PRICE.ITEMNO = ID.ITEM
WHERE IH.INV_DATE > 20250729  -- Updated column name
